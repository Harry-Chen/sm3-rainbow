...
use sm3::rainbow::{RainbowIndex, RainbowChain};
...
fn main() {
    ...
    let mut chains: Vec<_> = (0..rainbow_count).into_par_iter().map(|i| {
        let head = sm3::rainbow::RainbowIndex(plaintext_lens[4] + i);
        let chain = sm3::rainbow::RainbowChain::from_index(head, charset, &range, plaintext_lens.as_ref(), 0, rainbow_chain_len, 0);
    ...
    (0..rainbow_chain_len).into_par_iter().for_each(|i| {
        ...
        let mut target_tail = RainbowIndex::from_hash(
            &target_hash, 0, *plaintext_lens.last().unwrap(), chain_offset as u32
        );
        // remaining steps: H, R_{o+1}, H, ..., R_{l-1}
        if i > 0 {
            target_tail = RainbowIndex::traverse_chain(
                target_tail, charset, &range, &plaintext_lens, chain_offset + 1,
                i, 0, |_, _, _| false
            );
        }
        debug!("Searching for step {} with target tail {:#018x}\n", i, target_tail.0);
        match chains.binary_search(&RainbowChain{head: RainbowIndex(0), tail: target_tail}) {
            Ok(match_idx) => {
                let match_chain = &chains[match_idx];
                info!("Found matching chain {} on step {}: {:?}\n", match_idx, i, match_chain);
                match match_chain.find_match(&target_hash, charset, &range, plaintext_lens.as_ref(), rainbow_chain_len, 0) {
                    Some(result) => {
